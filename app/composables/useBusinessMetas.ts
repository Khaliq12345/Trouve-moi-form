...
export const useBusinessMetas = (bizId?: Ref<string | null | undefined>) => {
  ...
  const { data: groupedMetas, refresh: refreshMetas, pending: pendingMetas } = useAsyncData<GroupedBizMeta>(
    cacheKey.value,
    async () => {
  ...
  const createMeta = async (meta: Omit<BizMetaItem, 'id' | 'biz_name' | 'biz_slug'>) => {
    const currentBizId = bizId?.value;
    ...
    const payload = {
      ...
      media: meta.link?.map((fileId: string) => ({ directus_files_id: fileId })) || [],
    };
  ...
  const updateMeta = async (id: string, meta: Partial<BizMetaItem>) => {
    ...
    if (meta.link !== undefined) {
      payload.media = meta.link.map((fileId: string) => ({ directus_files_id: fileId }));
    }
